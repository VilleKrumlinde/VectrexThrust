...
//Ville: Recording support

unsigned read8 (unsigned address);  //forward declaration

static unsigned char lastInput = 255,lastInputCount = 0;
static unsigned char recordBuffer[5000];
int recordI = 0;

void recordToBuffer() {
  if(lastInput != 255) { //Don't record on first call
    recordBuffer[recordI++]=lastInput;
    recordBuffer[recordI++]=lastInputCount;
  }
}
...
//joy horiz  $c81b <0=left >0=right

  if(read8(0xc812)) input|=1;
  if(read8(0xc813)) input|=2;
  if(read8(0xc814)) input|=4;
  if(read8(0xc814)) input|=8;

  //Left/right
  joy = read8(0xc81b);
  if(joy>=128) input|=16;
  else if(joy>0) input|=32;

  //Up/down
  joy = read8(0xc81c);
  if(joy>=128) input|=64;
  else if(joy>0) input|=128;

  if( (input!=lastInput) || (lastInputCount==255) ) {
    recordToBuffer();
    lastInput=input;
    lastInputCount=0;
  } else lastInputCount++;
...
void dumpRecord() {
  ...
  for(i=0;i<recordI;i++) {
    if((i&15)==0)
      ...
      fprintf( stream, ", " );
    fprintf( stream, "$%02x", recordBuffer[i] );
  }
  
  fclose(stream);
}

//Ville: End of recording support
...
unsigned e6809_sstep (unsigned irq_i, unsigned irq_f)
{
	...
	case 0x14:  //Ville: Unused instruction, record input
    record();
		break;
//	case 0x15:  //Ville: Unused instruction, record input
//		break;
